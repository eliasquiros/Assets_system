...
fecha_adquisicion. RN-001.3: precision por dias exactos, no meses completos —
un activo que empieza a mitad de mes deprecia ese mes de forma proporcional.
RN-001.6/.7: la acumulada nunca supera el costo; al llegar o superar la vida
util en dias, queda TOTALMENTE_DEPRECIADO con acumulada == costo exacto.
"""
...
def calcular_depreciacion(costo_original, vida_util_anios, fecha_inicio, hasta=None):
    ...
    (hoy por defecto), para un activo que costo `costo_original`, con
    `vida_util_anios` y que inicio su uso en `fecha_inicio`."""
